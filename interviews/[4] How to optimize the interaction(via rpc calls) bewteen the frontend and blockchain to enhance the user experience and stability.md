...
5. Once the transaction final event is consumed by the Transaction Submission Service, it gets the critical fields like txHash、status etc from the message and figrues out the trackingId by the mapping between trackingId and txHash. Finally, it sychronizes the status of the signed transaction to the frontend by trackingId via websocket
