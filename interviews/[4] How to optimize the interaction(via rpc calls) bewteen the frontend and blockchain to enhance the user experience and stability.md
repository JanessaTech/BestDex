...
4. If the call doesn't hit the the cache defined in API gatway, check if the can be merged
5. Return at once if it can be merged
6. If it can not be merged, call backend API to check the cache in backend. It it hits, return the value at once. If it doesn't, call rpc to get the value from blockchain
7. All the response returned the backend should add information regarding 'Cache-Control' at API gateway, which will be used by frontend to set caching strategy
8. Websocket pushes the updates coming from monitoring events in chain to the frontend and updates the cache in the frontend

**[Interviewer]** Tell me more about optimizing rpc calls to send transaction to address issues: 1. no feedback when a transation is sent 2. the transation state is unknow while in waiting 3. no clear guide how to do next when the transaction fails

**[Me]** The core is:
- Once a transaction is sent to a delicated transaction in backend by the frontend, a status tracking id is returned immediately
- The transaction service sends the singed transaction asynchronously
- Combined with API to perform request merging / degradation / circuit-breaking process
-  The transaction service gets the update regarding the transaction from the data indexer and synchroinzes the update to frontend
...
2. The transaction service generates a ***trakingId*** for the signed transaction and returns it to the frontend immediately. Then it send a message consisting the trakingId and the signed transaction to a message queue
3. A consumer consume a message from the queue and then broadcasts the the signed transaction to get ***txHash***. We create a mapping between trakingId and txHash
4. The data indexer is monitoring both target events and receipts. A transaction final event is created after parsing the receipts. The transaction final event includes: txHash, status(0 / 1), blockNumber and gasUsed etc. This event is sent to the message queue waiting for the transaction service to consume it 
5. Once the transaction final event is consumed by the transaction service, it gets the critical fields like txHash、status etc from the message and figrues out the trackingId by the mapping between trackingId and txHash. Finally, it sychronizes the status of the signed transaction to the frontend by trackingId
...
- updating caching strategy accoring to the response returned by API gateway
- Setting up websocket connection and updating cache after a push arrives
