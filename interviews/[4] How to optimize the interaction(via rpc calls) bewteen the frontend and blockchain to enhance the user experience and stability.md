...
**[Me]** The core is:
- Once a transaction is sent to a delicated Transaction Submission Service in backend by the frontend, a status tracking id is returned immediately
- The Transaction Submission Service sends the singed transaction asynchronously
- The Transaction Submission Service gets the update regarding the transaction from the data indexer and synchroinzes the update to frontend via websocket

**[Interviewer]** Could you tell me more about the Transaction Submission Service? How does it work?

**[Me]** The Transaction Submission Service is used to send the signed transaction to blockchain and listens to the update regarding this signed transaction via the data indexer which is the data infrastructure system used to monitor events and receipts in chain. Once the update arrives, the Transaction Submission Service synchronizes the update to the frontend via websocket

Here is the data flow of how the Transaction Submission Service works combined with the frontend:
1. The frontend submits a signed transaction to the Transaction Submission Service
2. The Transaction Submission Service generates a ***trakingId*** for the signed transaction and returns it to the frontend immediately. Then it sends a message consisting the trakingId and the signed transaction to a message queue
...
5. Once the transaction final event is consumed by the Transaction Submission Service, it gets the critical fields like txHash、status etc from the message and gets trackingId by looking up the mapping between trackingId and txHash. Finally, it uses the websocket to sychronize the status the signed transaction to the frontend with trackingId as the indicator


**[Interviewer]** Execellent! you described all details regarding the Transaction Submission Service, the design is almost ready to get coded. One more question: do you think it is necessary to package all steps of what we did to optimize the rpc calls into a SDK used by the frontend?
...
- The Transaction Submission Service plays a middle-man role to send transaction and synchronize the updates
