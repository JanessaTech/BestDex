...
- **event monitor**:
    design monitor cluster with multiple monitor instances. Each instance is connected to a connection pool which manages multiple rpc providers or self-built nodes
...
- **processing**:
    there are multiple processing consumers to process messages in the message queue. The processed result is saved to redis to check the latest pool data and database for the history records.
...
2. **The united data consumption and pushing**
    Now we have the real time service to monitor events on the chain and update  data on the off chain accordingly, meanwhile, the frontend needs the updates to render page. I want to know how you expose the updates from the real time data service to the frontend?
...
**[Interviewer]** OK. Thanks for your clarification. Our aim of our design becomes clearer now: under the precondition of AlphaRouter working as a blackbox, how do you design a system which could provide an united data service used for both Swap quotes service and liquidity management service? and how do you share the data/updates to frontend in a timely way?

**[Me]**. OK. Let me take a look at first one: providing an united data service. After that, I will move on the solution to 'sharing the data/updates to frontend in a timely way'
...
- **precheck:**
    Assume we setup a united data query API which could query the latest pool data. Before calling AlphaRouter to get quotes, I could user this API to check if the current pool for the chosen token pair is valid, eg: if the liquidity is depleted or not. If it is invalid, no need to call AlphaRouter in the fllowing step
- **short-circuit calculation:**
    The frequent and simple token pairs, we could use the pool data to directly calculate the amount of the target token avoiding running AlphaRouter to quotes
- **slippage prediction:** 
    Since we store histry records of pool data, we could use them to predict the slippage to optimize the frontend user experience and give warning to users when the slippage is over the threshold
