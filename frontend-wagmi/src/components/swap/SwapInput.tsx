        ...
        <div className={`relative w-3/5 h-full box-border bg-zinc-900 border-zinc-700 border-[1px] rounded-r-md focus:border-pink-600 ${hidden ? 'hidden' : ''}`}>
            ...
            onKeyDown={(event) => {
                const allow =  (event?.key === 'Backspace' || event?.key === 'Delete')
                            || (input === '' && event?.key >= '0' && event?.key <= '9')
                            || (input === '0' && event?.key === '.')
                            || (/^[1-9]\d{0,20}$/.test(input) && event?.key >= '0' && event?.key <= '9')
                            || (/^[1-9]\d{0,20}$/.test(input) && event?.key === '.')
                            || (/^[1-9]\d{0,20}.\d{0,20}$/.test(input) && event?.key >= '0' && event?.key <= '9')
                            || (/^0.\d{0,20}$/.test(input) && event?.key >= '0' && event?.key <= '9')
                if (!allow) {
