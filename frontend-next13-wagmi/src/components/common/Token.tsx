...
const Token:React.FC<TokenProps> = ({token, className = 'flex items-center', imageSize, textSize = 'text-sm', defaultLabel = 'N/A'}) => {
    return (
        <div className={className}>
            {
                token ?<><Image src={`/imgs/tokens/${token?.name}.png`} alt='eth'width={imageSize} height={imageSize} className={`min-w-[${imageSize}px] rounded-full`}/><span className={`mx-2 truncate min-w-10 ${textSize}`}>{token.label}</span></> 
