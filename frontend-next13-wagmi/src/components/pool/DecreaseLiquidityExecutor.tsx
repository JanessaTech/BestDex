...
const DecreaseLiquidityExecutor: React.FC<DecreaseLiquidityExecutorType> = ({data, token0, token1, handleDecreaseLiquiditySuccess}) => {
    ...
    return (
        <div className="border-t-[1px] border-zinc-600 my-4 py-3 flex flex-col gap-y-1">
            <SimulateDecreaseLiquidityStep started={step === 1} done={step >= 1} skip={false} 
                parsedCalldata={data.parsedCalldata}
                goNext={goNext}/>
            <Seperator/>
            <DecreaseLiquidityStep token0={token0} token1={token1}
                started={step === 2}
