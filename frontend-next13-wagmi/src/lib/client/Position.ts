...
export const getPositionsByPage = async (chainId: number, owner: `0x${string}`, page: number = 1) => {
    logger.debug('[API client: Position] getPositionsByPage', 'chainId=', chainId, 'owner=', owner, 'page=', page)
    try {
        ...
        logger.error('[API client: Position] getPositionsByPage.', messageHelper.getMessage('positions_get_failed', chainId, owner, reason))
        logger.error(error)
        throw error
    }
}