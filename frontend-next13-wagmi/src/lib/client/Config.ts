...
import { ConfiguredTokens, DexResponseType, ConfiguredFeeTier } from './types'

export const getTokenListFromConfig = async () => {
    logger.debug('[API client: config] getTokenListFromConfig.')
    try {
        const response = await axios.get<DexResponseType<ConfiguredTokens[]>>(`${process.env.NEXT_PUBLIC_BACKEND_ADDR}/apis/v1/config/tokenList`)
...
export const getFeeTiersFromConfig = async () => {
    ...
    try {
        const response = await axios.get<DexResponseType<ConfiguredFeeTier[]>>(`${process.env.NEXT_PUBLIC_BACKEND_ADDR}/apis/v1/config/feetiers`)
        return response?.data?.data
    } catch (error:any) {
        const reason = error?.response?.data?.message || error?.message || error
