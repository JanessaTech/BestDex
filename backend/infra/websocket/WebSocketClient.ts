...
class WebSocketClient {
    ...
    private addAllWebSocketListeners = async (chainId: number) => {
            ...
            for (let i = 0; i < tokens.length; i++) {
                for (let j = i + 1; j < tokens.length; j++) {
                    ...
                    for (let k = 0; k < FEE_TIERS.length; k++) {
                        ...
                        try {
                            await this.addWebSocketListener(token0, token1, feeAmount, publicClient)
                            addedListeners++
                        } catch (error) {
                            logger.error(error)
                        }
    ...
    constructor(config: WebsocketConfig) {
        logger.debug('constructing WebSocket client...')
